main = interact $
  show . solve2 . tupp . (map $ parseDirs . words) . lines
...
solve :: (Dirs, Dirs) -> Int
solve (path0,path1) = minimum norms
  where
...
solve2 :: (Dirs,Dirs) -> Int
solve2 ds@(d0,d1) = k0
  where
    k0 = minimum $ map oneNorm $ firstIntsRelax ds 1430
    ints =  tail [x | x <- followPathBound d0 (0,0) k0, 
                      y <- followPathBound d1 (0,0) k0,
                      x == y]
    norms = map oneNorm ints


-- 1e200 is placeholder for INF
solve3 :: (Dirs,Dirs) -> Int
solve3 (d0,d1) = consume ([],[],1000000000000000) paths
  where
    paths = (followPath d0 (0,0), followPath d1 (0,0))
